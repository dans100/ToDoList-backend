...
export class UserRecord implements UserEntity {
    ...
    constructor(obj: UserEntity) {
        ...
        if (!obj.username || !obj.password || !obj.email) {
            throw new ValidationError("Please enter a valid email, username and password (non-empty values).")
        }

        if (obj.username.length > 55 || obj.username.length < 3) {
            throw new ValidationError('User name cannot be shorter than 3 characters and later than 55 characters');
        }

        if (obj.password.length > 60 || obj.password.length < 7) {
            throw new ValidationError('Password cannot be shorter than 7 characters and later than 60 characters');
        }
    ...
    public async insert(): Promise<string> {
        ...
        await pool.execute('INSERT INTO `users` VALUES(:id, :username, :password, :email)', {
            id: this.id,
