...
export class TaskRecord implements TaskEntity {
  ...
  constructor(public obj: TaskEntity) {
    if (
      !obj.description ||
      obj.description.length < 3 ||
      obj.description.length > 55
    ) {
  ...
  public async insert(): Promise<void> {
    ...
    await pool.execute(
      'INSERT INTO `tasks` VALUES(:id, :description, :user_id, :isCompleted, :deadline)',
      {
        id: this.id,
  ...
  public async update(): Promise<void> {
    await pool.execute(
      'UPDATE `tasks` SET `description`=:description, `isCompleted`=:isCompleted WHERE `id`=:id',
      {
        id: this.id,
        description: this.description,
  ...
  public static async listAll(
    ...
    const [result] = (await pool.execute(
      'SELECT * FROM `tasks` WHERE `user_id`=:user_id AND `description` LIKE :search',
      {
        search: `%${name}%`,
        user_id: userId,
      }
  ...
  public static async deleteComplete(userId: string): Promise<void> {
    await pool.execute(
      'DELETE FROM `tasks` WHERE `user_id`=:user_id AND `isCompleted`=:isCompleted',
      {
        user_id: userId,
        isCompleted: 1,
      }
